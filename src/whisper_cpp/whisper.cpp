...
static bool kv_cache_init(
    ...
    if (!cache.ctx) {
        Rprintf("%s: failed to allocate memory for kv cache\n", __func__);
        return false;
    }
...
static bool whisper_model_load(struct whisper_model_loader * loader, whisper_context & wctx) {
    ...
    // verify magic
    {
        ...
        if (magic != 0x67676d6c) {
            Rprintf("%s: invalid model data (bad magic)\n", __func__);
            return false;
        }
    }
    ...
    //load hparams
    {
        ...
        // print memory requirements
        {
            ...
            Rprintf("%s: mem required  = %7.2f MB (+ %7.2f MB per decoder)\n", __func__,
    ...
    // load vocab
    {
        ...
        for (int i = 0; i < n_vocab; i++) {
            ...
            if (len > 0) {
                ...
                //Rprintf("%s: warning: empty-string token in vocab, i = %d\n", __func__, i);
                word = "";
            }
...
struct whisper_context * whisper_init_from_buffer(void * buffer, size_t buffer_size) {
    ...
    Rprintf("%s: loading model from buffer\n", __func__);
...
struct whisper_context * whisper_init(struct whisper_model_loader * loader) {
    ...
    if (!whisper_model_load(loader, *ctx)) {
        ...
        Rprintf("%s: failed to load model\n", __func__);
        delete ctx;
        return nullptr;
    }
...
const char * whisper_lang_str(int id) {
    ...
    Rprintf("%s: unknown language id %d\n", __func__, id);
    return nullptr;
}
...
int whisper_lang_auto_detect(
    ...
    if (seek < 0) {
        Rprintf("%s: offset %dms is before the start of the audio\n", __func__, offset_ms);
        return -1;
    }

    if (seek >= ctx->mel.n_len) {
        Rprintf("%s: offset %dms is past the end of the audio (%dms)\n", __func__, offset_ms, ctx->mel.n_len*10);
        return -2;
    }
...
static void whisper_process_logits(
    ...
    // ref: https://github.com/openai/whisper/blob/0b1ba3d46ebf7fe6f953acfd8cad62a4f851b49f/whisper/decoding.py#L480-L493
    {
        ...
        // https://github.com/openai/whisper/blob/0b1ba3d46ebf7fe6f953acfd8cad62a4f851b49f/whisper/decoding.py#L414-L424
        {
            ...
            //Rprintf("last_was_timestamp=%d penultimate_was_timestamp=%d\n", last_was_timestamp, penultimate_was_timestamp);
        ...
        // ref: https://github.com/openai/whisper/blob/0b1ba3d46ebf7fe6f953acfd8cad62a4f851b49f/whisper/decoding.py#L431-L437
        {
            ...
            //Rprintf("timestamp_logprob=%f max_text_token_logprob=%f\n", timestamp_logprob, max_text_token_logprob);
...
int whisper_full(
    ...
    if (params.language == nullptr || strlen(params.language) == 0 || strcmp(params.language, "auto") == 0) {
        ...
        if (lang_id < 0) {
            Rprintf("%s: failed to auto-detect language\n", __func__);
            return -3;
        }
        ctx->lang_id = lang_id;
        params.language = whisper_lang_str(lang_id);

        Rprintf("%s: auto-detected language: %s (p = %f)\n", __func__, params.language, probs[whisper_lang_id(params.language)]);
    }
    ...
    for (int j = 1; j < n_decoders; j++) {
        ...
        if (decoder.kv_self.ctx == nullptr) {
            ...
            if (!kv_cache_reinit(decoder.kv_self)) {
                Rprintf("%s: kv_cache_reinit() failed for self-attention, decoder %d\n", __func__, j);
                return -4;
            }
    ...
    if (params.audio_ctx > whisper_n_audio_ctx(ctx)) {
        Rprintf("%s: audio_ctx is larger than the maximum allowed (%d > %d)\n", __func__, params.audio_ctx, whisper_n_audio_ctx(ctx));
        return -5;
    }
...
int whisper_full_parallel(
    ...
    for (int i = 0; i < n_processors - 1; ++i) {
        ...
        if (!kv_cache_reinit(ctx_p.kv_cross)) {
            Rprintf("%s: kv_cache_reinit() failed for cross-attention, processor %d\n", __func__, i);
            return false;
        }
        ...
        for (int j = 0; j < WHISPER_MAX_DECODERS; ++j) {
            if (ctx_p.decoders[j].kv_self.ctx && !kv_cache_reinit(ctx_p.decoders[j].kv_self)) {
                Rprintf("%s: kv_cache_reinit() failed for self-attention, decoder %d, processor %d\n", __func__, j, i);
                return false;
            }
...
WHISPER_API int whisper_bench_ggml_mul_mat(int n_threads) {
    ...
    for (int j = 0; j < (int) sizes.size(); j++) {
        ...
        Rprintf("ggml_mul_mat: %5zu x %5zu: F16 %8.1f GFLOPS (%3d runs) / F32 %8.1f GFLOPS (%3d runs)\n",
